...
# Install EvtxEcmd
# ----------------------------------------------------------------------
ENV EVTXECMD_ZIP="EvtxECmd.zip"

# Install .net 9 as Linux compatible
# Download the specified Hayabusa release using curl
RUN wget https://download.ericzimmermanstools.com/net9/EvtxECmd.zip -O ${EVTXECMD_ZIP} --no-check-certificate
# Unzip and clean up
RUN unzip ${EVTXECMD_ZIP} -d /evtxecmd && rm ${EVTXECMD_ZIP}

# extracts contents to
# /evtxecmd/EvtxeCmd/Maps/
# /evtxecmd/EvtxeCmd/EvtxECmd.dll
# /evtxecmd/EvtxeCmd/EvtxECmd.exe
# /evtxecmd/EvtxeCmd/EvtxECmd.runtimeconfig.json

# ----------------------------------------------------------------------

# Default command if not run from docker-compose (and command being overidden)
CMD ["celery", "--app=openrelik_worker_evtxecmd.tasks", "worker", "--task-events", "--concurrency=1", "--loglevel=INFO"]